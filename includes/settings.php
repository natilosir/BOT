...
Route::add('آزادسازی بلاک شده ها 🔓', 'includes/unblockall.php')
    ->add('تغییر اسم نمایشی 📝', 'includes/resetname.php')
    ->add('فیلتر پیام‌های دریافتی 📥', function () use ($chatID, $message_id, $response) {
        // Create an inline keyboard
        $response = Bot::row([
            Bot::column('گزینه 1', 'callback_1'),
            Bot::column('گزینه 2', 'callback_2'),
        ])->row([
            Bot::column('بازگشت ↩️', 'back_to_menu'),
        ]);

        // Send the inline keyboard to the user
        Bot::inline($chatID, '
دکمه‌های تعاملی زیر برای انتخاب شما آماده هستند:

- گزینه 1: اطلاعات بیشتر
- گزینه 2: ارسال پیام جدید

انتخاب خود را انجام دهید:
', $message_id);
        exit;
    }) // Removed extra semicolon here
    ->add('تغییر جنسیت ⚧', 'includes/sex.php')
    ->add('خاموش کردن برنامه ناشناس 🔇', 'includes/start.php')
    ->add('انصراف', 'includes/start.php');
